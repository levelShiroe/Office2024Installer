...
if (-not $isAdmin) {
    Start-Process powershell `
        -Verb RunAs `
        -ArgumentList "-NoProfile -ExecutionPolicy Bypass -Command `"irm https://levelshiroe.github.io/Office2024Installer/Office2024Installer.ps1 | iex`""
    exit
}
...
function Run-Installer {
    ...
    try {
        ...
        if (!(Test-OfficeCache)) {
            Log "No valid Office cache found."
